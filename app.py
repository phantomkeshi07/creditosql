...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mssql+pyodbc://sa:130197@localhost/creditobanco?driver=ODBC+Driver+17+for+SQL+Server'
db = SQLAlchemy(app)
...
# Rota para fornecer os dados como API JSON
@app.route('/api/dados')
def api_dados():
    atualizar_dados()  # Atualiza os dados antes de exibir
    dados = Dados.query.all()
    dados_json = [
        {
            ...
            'assessor_descricao': dado.assessor_descricao,
            'produto_descricao': dado.produto_descricao,
            'tabela_descricao': dado.tabela_descricao,
            'status_descricao': dado.status_descricao,
            'data_lancamento': dado.data_lancamento,
            'valor_emprestimo': dado.valor_emprestimo,
            'valor_parcelas': dado.valor_parcelas,
            'numero_parcelas': dado.numero_parcelas,
            'tipo_midia_descricao': dado.tipo_midia_descricao,
            'banco_descricao': dado.banco_descricao,
            'banco_ispb': dado.banco_ispb
        }
        for dado in dados
    ]
